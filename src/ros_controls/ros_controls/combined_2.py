...
class UnifiedDetectionNode(Node):
    def __init__(self):
        ...
        # Clean, explicit Reliable QoS Matching your updated publisher stream
        qos = QoSProfile(
        ...
        self.get_logger().info('UnifiedDetectionNode started with Dual-Mask Red HSV tracking.')
    ...
    def get_hsv_bboxes(self, frame_bgr):
        ...
        # Combine both regions to catch entire wrapping red hue
    ...
    def synced_callback(self, img_msg, depth_msg):
        # Force writeable array copies to prevent silent drawing runtime bugs
        ...
        for (x1, y1, x2, y2, conf, cls_name) in yolo_dets:
            bbox    = (x1, y1, x2, y2)
            # Use class name in track key to prevent sorting-order noise from mixing up filters
