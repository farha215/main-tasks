...
class UnifiedDetectionNode(Node):
    def __init__(self):
        ...
        # Clean, explicit Reliable QoS Matching your publisher stream
        qos = QoSProfile(
        ...
        # ZED SDK objects (Now contains the native YOLO detections)
        ...
        self.get_logger().info('UnifiedDetectionNode started (Native ZED SDK Inference + HSV).')
    ...
    def objects_callback(self, msg):
        # This now contains the native YOLO output from the ZED Wrapper
        self.zed_objects = msg.objects

    # ??? HSV Double Mask Detection ?????????????????????????????????????????????
    ...
    def synced_callback(self, img_msg, depth_msg):
        ...
        # Make a thread-safe copy of the latest ZED objects
        ...
        for obj in current_zed_objects:
            ...
            # Extract 2D Bounding Box from ZED Object
            ...
            # Extract labels and 3D data natively calculated by ZED
            cls_id   = obj.label_id
            cls_name = YOLO_CLASS_MAP.get(cls_id, f"class_{cls_id}")
            conf     = float(obj.confidence) / 100.0
            
            pos      = (float(obj.position[0]), float(obj.position[1]), float(obj.position[2]))
            distance = float(obj.position[0]) # ZED provides highly accurate 3D spatial depth

            obj_id  = f'yolo_{cls_name}_{tracking_id}'
            current_ids.add(obj_id)

            det_array.detections.append(
                self.build_custom_det(img_msg.header, bbox, cls_name, conf, distance, pos, tracking_id))
        ...
        for bbox in hsv_bboxes:
            ...
            # Since HSV isn't tracked by ZED natively, we fallback to depth map
            ...
            det_array.detections.append(
                self.build_custom_det(img_msg.header, bbox, 'preq_pole', 0.5, distance, pos, tracking_id))
            det3d_array.detections.append(
            ...
            self.get_logger().info(f'[HSV] preq_pole at {distance:.2f}m', throttle_duration_sec=2.0)
            tracking_id += 1
